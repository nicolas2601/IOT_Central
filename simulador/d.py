...
# Generado a partir de la plantilla seleccionada
...
def build_sample():
    # Construye una muestra de telemetría basada en la plantilla
    sample = {}
    sample['ph'] = round(random.uniform(0, 14), 1)
    sample['turbidity'] = round(random.uniform(0, 100), 2)
    sample['temperature'] = round(random.uniform(0, 50), 1)
    return sample

def main():
    parser = argparse.ArgumentParser(description='Simulador de telemetría MQTT')
    parser.add_argument('--device-id', type=str, default='ccf18fd5-e27b-4aec-a178-0bb216ce1ac9', help='ID del dispositivo (UUID)')
    ...
    client = mqtt.Client(client_id=f'simulator_{template.id}_{device_id}')
    try:
