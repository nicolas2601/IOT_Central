    ...
    <Card className="bg-black/40 border-white/10 backdrop-blur-xl">
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <div>
          ...
          <CardDescription>
            Ejecuta localmente con el ID del dispositivo; usa MQTT.
          </CardDescription>
        </div>
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          <div className="text-xs text-white/80 space-y-2">
            <p><b>Requisitos:</b> Python 3.8+, broker MQTT (ej. Mosquitto), librería <code>paho-mqtt</code>.</p>
...
python simular_telemetria.py --device-id ${deviceId} --broker-host localhost --broker-port 1883 --interval 2
`}</pre>
            <p className="text-white/70">Nota: si tu carpeta tiene espacios/acentos (ej. "Más Carpetas"), usa comillas como en el <code>cd</code>.</p>
...
python .\\simular_telemetria.py --device-id ${deviceId} --broker-host localhost --broker-port 1883 --interval 2
`}</pre>
            <p className="text-white/70">Si PowerShell bloquea la activación, ejecuta (como usuario): <code>Set-ExecutionPolicy RemoteSigned -Scope CurrentUser</code>.</p>
...
`}</pre>
            ...
            <pre className="bg-black/50 border border-white/10 rounded p-2 whitespace-pre-wrap">{`
mosquitto_sub -t dispositivo/${deviceId}/telemetria -h localhost -p 1883
`}</pre>
            <p className="text-white/70">En otra ventana ejecuta el simulador; deberías ver mensajes JSON publicados.</p>
            ...
            <ul className="list-disc pl-5 text-white/70 space-y-1">
              ...
              <li>Si no conecta al broker, asegúrate de que Mosquitto está corriendo en <code>localhost:1883</code>.</li>
              <li>Si tu antivirus/firewall bloquea MQTT, habilita localhost para pruebas.</li>
            </ul>

            <p className="text-white/80 mt-2">Comando final (rápido):</p>
            <pre className="bg-black/50 border border-white/10 rounded p-2 whitespace-pre-wrap">{`python simular_telemetria.py --device-id ${deviceId} --broker-host localhost --broker-port 1883 --interval 2`}</pre>
          </div>
